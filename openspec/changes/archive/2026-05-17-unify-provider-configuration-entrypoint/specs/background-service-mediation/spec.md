...
The background service worker SHALL be the only extension component that calls external Agent, LLM, embedding, localhost gateway, or memory repository APIs for knowledge explanation, ambiguity judgment, rewrite generation, embedding, health, diagnostics, and memory-backed Agent context.
...
- **THEN** it SHALL include only selected text, minimal surrounding context, sanitized user memory summary, request goal, provider protocol metadata, configured model name when present, and policy metadata needed for the explanation
...
- **THEN** the background service worker SHALL resolve the active provider role and route the request without requiring content script changes.

### Requirement: Background Owns Provider Configuration
The background service worker SHALL own provider configuration, token reading, local pairing token reading, endpoint selection, model dispatch, timeout selection, health checks, host permissions, and capability discovery for Agent, LLM, gateway, memory, and embedding services.

#### Scenario: Explain provider token is needed
- **GIVEN** an explain or rewrite request requires a provider token
- **WHEN** the request is sent
- **THEN** the background service worker SHALL read and apply the explain provider token
- **AND** the content script SHALL NOT receive or persist the credential.

#### Scenario: Embedding provider token is needed
- **GIVEN** an embedding request requires a provider token
- **WHEN** the request is sent
- **THEN** the background service worker SHALL read and apply the embedding provider token
- **AND** the content script SHALL NOT receive or persist the credential.
...
- **WHEN** the request is sent
- **THEN** the background service worker SHALL read and apply the top-level local gateway pairing token
- **AND** the content script SHALL NOT receive or persist the token.

#### Scenario: Provider is not configured
- **GIVEN** no provider is configured for the requested role or the provider mode is `off`
- **WHEN** the background service worker receives a proactive explanation, rewrite, or embedding request
...
- **GIVEN** a provider role is configured
...
- **AND** it SHALL associate the result with the provider role and configured model name when present.

### Requirement: Background Controls External Service Reliability
The background service worker SHALL enforce role-specific timeouts, cache policy, rate limits, structured error handling, capability checks, local gateway unavailable handling, and privacy policy for external service and localhost gateway calls.

#### Scenario: Agent request times out
- **GIVEN** an external Agent request exceeds the configured explain timeout
...
The background service worker SHALL mediate all optional embedding service calls used for similar-memory retrieval through the embedding provider role.
...
- **THEN** the background service worker SHALL call the embedding provider with the configured embedding token and model name when present
